    ...
    <div className="flex flex-wrap flex-col -m-4">
      {marketsArray.map((market) => (
        <div key={market.adj_ticker} className="p-4 w-full">
          <div className="w-full md:max-w-none border border-gray-200 rounded-lg overflow-hidden shadow-sm hover:shadow-md transition-shadow duration-300" style={{ height: "150px", width: "auto", maxWidth: "750px" }}>
            <a href={`https://data.adj.news/explore/market?question=${market.ticker}`} target="_blank" rel="noopener noreferrer" className="block">
              <div className="flex">
                <div className="w-1/3">
                  <img src={`https://source.boringavatars.com/marble/150/${market.question ? market.question : market.ticker}?square`} alt={market.ticker} className="object-cover w-full h-full" />
                </div>
                <div className={`p-4 w-2/3`}>
                  <h2 className="font-bold text-lg mb-2 text-gray-800">{market.question ? market.question : market.ticker}</h2>
                  <p className="text-sm text-gray-600 mb-2">{`${market.probability}%`} probability</p>
